...
function addDays(d: Date, days: number) { const next = new Date(d); next.setDate(next.getDate() + days); return next; }
function startOfWeek(d: Date) { const next = new Date(d); const day = next.getDay(); const diff = day === 0 ? -6 : 1 - day; next.setDate(next.getDate() + diff); return next; }
function startOfMonth(month: string) { return `${month}-01`; }
function endOfMonth(month: string) {
  const [year, m] = month.split('-').map(Number);
  return dateString(new Date(year, m, 0));
}
function isSameDateRange(from: string, to: string) { return from === to; }
...
export function AttendancePage({ onlyMe = false }: { onlyMe?: boolean }) {
  ...
  useEffect(() => {
    ...
    if (month === browserMonth) setMonth(currentMonth);
    if (detailMonth === browserMonth) setDetailMonth(currentMonth);
  ...
  const personalSummaryEndpoint = useMemo(() => {
    if (!onlyMe || !user?.employeeId) return '';
    const params = new URLSearchParams({ from: startOfMonth(month), to: endOfMonth(month) });
    return `/attendances/employee/${user.employeeId}/summary?${params.toString()}`;
  }, [month, onlyMe, user?.employeeId]);

  const detailSummaryEndpoint = useMemo(() => {
    if (!detailEmployee?.employeeId) return '';
    const params = new URLSearchParams({ from: startOfMonth(detailMonth), to: endOfMonth(detailMonth) });
  ...
  function enableRangeMode() {
    if (from === to) setTo(endOfMonth(from.slice(0, 7)));
  }
    ...
    return (
      <section>
        ...
        <div className="card filters month-toolbar">
          <label>Tháng hiện tại / tháng cần xem<input type="month" value={month} onChange={(e) => setMonth(e.target.value)} /></label>
        ...
        <MonthlyCalendar rows={personalRows} month={month} loading={personalLoading} todayValue={today} />
      </section>
    );
  }
    ...
    <section>
      ...
      <div className="card attendance-filter-card">
        <div className="date-shortcuts">
          <button type="button" className="secondary" onClick={() => setSingleDate(today)}>Hôm nay</button>
          <button type="button" className="secondary" onClick={() => setSingleDate(dateString(addDays(new Date(), -1)))}>Hôm qua</button>
          <button type="button" className="secondary" onClick={() => setRange(dateString(startOfWeek(new Date())), today)}>Tuần này</button>
          <button type="button" className="secondary" onClick={() => setRange(startOfMonth(currentMonth), today)}>Tháng này</button>
        </div>
        <div className="filters">
          <div className="segmented-control">
            <button type="button" className={!rangeMode ? 'active' : ''} onClick={() => setTo(from)}>Xem 1 ngày</button>
            <button type="button" className={rangeMode ? 'active' : ''} onClick={enableRangeMode}>Xem khoảng ngày</button>
          </div>
          {!rangeMode ? (
            <label>Ngày cần xem<input type="date" value={from} onChange={(e) => setSingleDate(e.target.value)} /></label>
          ) : (
            <>
      ...
      {detailEmployee && <div className="modal-backdrop" onClick={() => setDetailEmployee(null)}>
        <div className="modal-card wide-modal" onClick={(e) => e.stopPropagation()}>
          <div className="modal-title-row">
            ...
            <button className="secondary" onClick={() => setDetailEmployee(null)}>Đóng</button>
          </div>
          <div className="filters compact-filter month-toolbar">
            <label>Tháng<input type="month" value={detailMonth} onChange={(e) => setDetailMonth(e.target.value)} /></label>
            <MonthSummaryPanel summary={detailSummaryRows[0]} />
          </div>
          <MonthlyCalendar rows={detailRows} month={detailMonth} loading={detailLoading} compact todayValue={today} />
        </div>
      </div>}
    </section>
  );
...
function DailyTable({ rows, loading }: { rows: Attendance[]; loading: boolean }) {
  return <div className="card table-wrap">
    <h3>Bảng công ngày đang chọn</h3>
    {loading ? <p>Đang tải...</p> : <table>
      <thead><tr><th>Nhân viên</th><th>Team</th><th>Loại ngày</th><th>Lịch/Ca</th><th>Giờ chuẩn</th><th>Check-in</th><th>Break out</th><th>Break in</th><th>Check-out</th><th>Phút làm</th><th>Nghỉ</th><th>Muộn</th><th>Về sớm</th><th>OT</th><th>Trạng thái</th></tr></thead>
      <tbody>{rows.map((row) => <tr key={row.id ?? `${row.employeeId}-${row.workDate}`} className={row.holiday ? 'holiday-row' : row.weekend ? 'weekend-row' : ''}>
        ...
        <td>{row.weekend || row.holiday ? <span className="muted-text">-</span> : <span className="badge">{viLabel(row.status)}</span>}</td>
      </tr>)}</tbody>
    </table>}
  </div>;
}
...
function MonthSummaryPanel({ summary }: { summary?: AttendanceSummary }) {
  return <div className="month-summary-panel">
...
function SummaryTable({ rows, loading, onOpen }: { rows: AttendanceSummary[]; loading: boolean; onOpen: (row: AttendanceSummary) => void }) {
  return <div className="card table-wrap">
    <h3>Tổng hợp theo nhân viên</h3>
    <p className="hint">Click vào một nhân viên để xem bảng công theo tháng.</p>
    {loading ? <p>Đang tải...</p> : <table>
      <thead><tr><th>Nhân viên</th><th>Team</th><th>Ngày chuẩn</th><th>Ngày nghỉ</th><th>Ngày có công</th><th>Vắng</th><th>Đi muộn</th><th>Về sớm</th><th>Thiếu check</th><th>Tổng giờ làm</th><th>Tổng OT</th><th>Tổng muộn</th><th>Tổng về sớm</th><th>Phép đã dùng</th><th>Phép còn</th></tr></thead>
      <tbody>{rows.map((row) => <tr key={row.employeeId} className="clickable-row" onClick={() => onOpen(row)}>
        <td>{employeeLabel(row)}</td>
        <td>{codeName(row.teamCode, row.teamName)}</td>
...
function MonthlyCalendar({ rows, month, loading, compact = false, todayValue }: { rows: Attendance[]; month: string; loading: boolean; compact?: boolean; todayValue?: string }) {
  const rowMap = useMemo(() => new Map(rows.map((row) => [row.workDate, row])), [rows]);
  ...
  const days = useMemo(() => {
    ...
    for (let i = 0; i < leadingBlank; i++) cells.push({});
    for (let day = 1; day <= lastDay; day++) {
      const date = `${month}-${String(day).padStart(2, '0')}`;
      cells.push({ date, row: rowMap.get(date) });
    }
    return cells;
  }, [month, rowMap]);
  ...
  return <div className="card attendance-calendar-card">
    ...
    <div className={`attendance-calendar ${compact ? 'compact-calendar' : ''}`}>
      {days.map((cell, index) => {
        ...
        return <div key={cell.date} className={`calendar-day ${dayClass} ${todayClass}`}>
          ...
          <div className="calendar-lines">
            {isFuture ? (
              ...
              <>
                ...
                <span>Thời gian làm: {minutesToHourText(row?.totalWorkingMinutes)}</span>
                <span>OT ngày nghỉ: {minutesToHourText(row?.overtimeMinutes)}</span>
              </>
            ) : (
              <>
                <span>Lịch: {shortTime(row?.plannedStartTime)} - {shortTime(row?.plannedEndTime)}</span>
                <span>Vào/Ra: {shortTime(row?.firstCheckIn)} - {shortTime(row?.lastCheckOut)}</span>
                <span>Nghỉ: {shortTime(row?.lastBreakOut)} - {shortTime(row?.firstBreakIn)}</span>
                <span>Làm: {minutesToHourText(row?.totalWorkingMinutes)}</span>
                <span>Muộn: {row?.lateMinutes ?? 0}p · Sớm: {row?.earlyLeaveMinutes ?? 0}p</span>
...
function dayTypeLabel(row: Pick<Attendance, 'holiday' | 'weekend' | 'holidayName'>) {
  if (row.holiday) return row.holidayName ? `Nghỉ lễ: ${row.holidayName}` : 'Nghỉ lễ';
  if (row.weekend) return 'Thứ 7 / Chủ nhật';
  return 'Ngày làm';
}
